...
function Invoke-ESAFActiveDirectoryAuditAssessment {
    ...
    function Add-ADFinding {
        ...
        # تغییر محدوده متغیر به اسکوپ والد (تابع Invoke-ESAFActiveDirectoryAuditAssessment)
        $GetFinding = New-ESAFFinding `
            -FindingID $FindingID `
            -Category $Category `
            -Title $Title `
            -Severity $Severity `
        ...
        $script:findings += $GetFinding # برای پشتیبانی از رفتارهای سنتی
        if ($null -eq (Get-Variable -Name "findings" -Scope 1 -ErrorAction SilentlyContinue)) {
             # Fallback
        } else {
             Set-Variable -Name "findings" -Value ($findings + $GetFinding) -Scope 1
        }
    }
